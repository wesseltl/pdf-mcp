...
def validate_offer(path: Path, offer: dict) -> list[str]:
    ...
    if handling.get("working_files_deleted_days_after_delivery") != 14:
        errors.append(f"{label}: working-file retention must match the published 14-day policy")
    ...
    if checkout_url is None:
        if offer.get("status") != "accepting_requests":
            errors.append(f"{label}: an offer without checkout must be accepting_requests")
        if not any(
    ...
    if not (seller.get("vat_id") or seller.get("vat_status")):
        errors.append(f"{label}: live checkout requires seller.vat_id or seller.vat_status")
    return errors
...
def validate_beta_offer(path: Path, offer: dict) -> list[str]:
    ...
    if offer.get("status") != "accepting_beta_requests":
        errors.append(f"{label}: beta must remain request-only until an endpoint is published")
    if offer.get("audience") != "businesses_and_professionals_only":
    ...
    if limits != expected_limits:
        errors.append(f"{label}: hosted beta limits do not match the published contract")
    ...
    methods = [
        method for method in offer.get("access_methods", [])
    ...
    for field in ("terms_url", "privacy_url", "human_readable_url", "local_unmeasured_alternative"):
        if urlparse(offer.get(field, "")).scheme != "https":
            errors.append(f"{label}: {field} must be an HTTPS URL")
    return errors


def validate_repository(require_live_checkout: bool = False) -> tuple[list[str], list[str]]:
    errors = []
    notes = []
    ...
    if BETA_PATH.is_file():
        ...
        if beta_offer.get("service", {}).get("url") is None:
            notes.append("free hosted beta is accepting requests; endpoint deployment is pending")
        else:
            notes.append("free hosted beta endpoint is published")

    live_offers = [offer for offer in offers if offer.get("checkout_url")]
    if live_offers:
        notes.append(f"live checkout configured for {len(live_offers)} of {len(offers)} offers")
        terms_path = ROOT / "docs" / "terms.html"
        terms_html = terms_path.read_text(encoding="utf-8") if terms_path.is_file() else ""
        if "checkout is disabled" in terms_html or "checkout remains disabled" in terms_html:
            errors.append("live checkout conflicts with the request-only statement in docs/terms.html")
...
def main() -> int:
    ...
    if errors:
        ...
        return 1
    print(
        f"Validated {len(OFFER_PATHS)} paid offers, 1 hosted beta offer, "
