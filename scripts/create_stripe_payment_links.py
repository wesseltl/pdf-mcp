...
def process_offer(
    ...
    if result["stripe_mode"] == "test":
        if write:
            raise SystemExit("Test checkout URLs cannot be written to published offer files.")
        return {"path": str(path), "status": "created_test", **result, "wrote_file": False}
...
def main() -> int:
    ...
    if args.live and args.expected_mode == "live" and not args.write:
        raise SystemExit("Live checkout creation requires --write so the URLs are not lost.")
